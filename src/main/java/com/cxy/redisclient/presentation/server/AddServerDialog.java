...
public class AddServerDialog extends RedisClientDialog {
	...
	protected void createContents() {
        ...
        Label lblClusterName = new Label(composite, SWT.NONE);
        lblClusterName.setText(RedisClient.i18nFile.getText(I18nFile.CLUSTERNAME));
		...
		btnOk.addMouseListener(new MouseAdapter() {
			...
			public void mouseUp(MouseEvent arg0) {
				String clusterName = text_2.getText();
				...
				if(name.length() == 0 || host.length() == 0 || port.length() == 0)
					...
					result = new Server(0, clusterName, name, host, port, password);
					shell.dispose();
				}
